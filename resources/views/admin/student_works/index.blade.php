    ...
    <table class="table">
        ...
        @foreach($studentWorks as $work)
            <tr>
                ...
                <td>
                    <a href="{{ route('admin.student-works.edit', $work->id) }}" class="btn btn-warning btn-sm">Edit</a>
                    <form action="{{ route('admin.student-works.destroy', $work->id) }}" method="POST" style="display:inline;">
                        @csrf @method('DELETE')
                        <button class="btn btn-danger btn-sm" onclick="return confirm('Hapus karya ini?')">Hapus</button>
                    </form>
                </td>
            </tr>
        @endforeach
