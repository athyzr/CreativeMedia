...
@section('content')
  ...
  <div class="container mx-auto px-4 py-10">
    <div class="bg-white p-6 rounded shadow">
      @if($service->image)
        <img src="{{ asset('storage/' . $service->image) }}" alt="{{ $service->title }}" class="w-full h-64 object-cover rounded mb-4">
      @endif
      <h1 class="text-2xl font-bold mb-2">{{ $service->title }}</h1>
      <div class="text-gray-700 prose max-w-none">{!! $service->description !!}</div>
    </div>
  </div>
@endsection
