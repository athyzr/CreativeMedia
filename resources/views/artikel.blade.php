...
<div class="bg-cyan-700 text-white text-center py-20 mt-20">
  <h1 class="text-4xl font-bold">Artikel Kami</h1>
  <p class="mt-4 text-lg">Dapatkan Informasi & Inspirasi Terbaru</p>
</div>
  ...
  <div class="grid md:grid-cols-3 gap-8">
    @forelse ($articles as $article)
      <div class="bg-white rounded-lg shadow hover:shadow-lg transition overflow-hidden">
        ...
        <img src="{{ asset('storage/' . $article->thumbnail) }}" alt="Artikel Image" class="h-48 w-full object-cover" />
        @else
          <img src="{{ asset('images/default-thumbnail.jpg') }}" alt="Default Thumbnail" class="h-48 w-full object-cover">
        @endif
        <div class="p-6">
          ...
          <a href="{{ route('articles.show', $article->id) }}" class="text-cyan-700 hover:underline font-semibold">Baca Selengkapnya</a>
          </div>
      </div>
    @empty
      <div class="col-span-3 text-center">
        <h2 class="text-gray-500 text-xl">Belum ada artikel yang tersedia.</h2>
      </div>
    @endforelse
  </div>
</div>
